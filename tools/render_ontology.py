...
STAGE_ROWS = [
    ...
    ("SLA (target / rotting)", lambda s: " / ".join(str((s.get("sla") or {}).get(k, "—"))
...
function stageDetail(s){
 const sla=s.sla?`target ${s.sla.target_duration_days??"—"} d · rotting after ${s.sla.rotting_threshold_days??"—"} d`:"";
