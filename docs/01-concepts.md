...
- **Fill authorship** (`filled_by`): `human | automation | ai | integration | system` — the single most important fact for an agent interpreting a value.
...
| A1 | `business-context.md` | 1 | Markdown + YAML frontmatter | ontology | — |
| A2 | `glossary.yaml` | 1 | YAML | ontology | — (see formats doc) |
| A3 | `objects/<id>.yaml` | 1 | YAML | object type | `object-type.schema.json` |
| A4 | `systems/<id>.yaml` | 2 | YAML | connected app | `system.schema.json` |
| A5 | `discovery/<system>/snapshot.yaml` | 2 | YAML | system × run | — (normalized envelope, system-specific body) |
| A6 | `mappings/<system>.yaml` | 2 | YAML | system | `binding.schema.json` |
| A7 | `identity.yaml` | 2 | YAML | ontology | — (see formats doc) |
...
| A11 | `prompts/<id>.md` | 3 | Markdown + YAML frontmatter | prompt | — |
| A12 | `kpis/<id>.yaml` | 4 | YAML | KPI (grouping allowed) | `kpi.schema.json` |
| A13 | `agent-policy.yaml` | 4 | YAML | ontology | — (see formats doc) |
| A14 | `drafts/<id>.md` | 3 | Markdown + YAML frontmatter | communication template | — (see formats doc) |
