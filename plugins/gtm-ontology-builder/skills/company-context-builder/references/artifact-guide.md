...
When consequential information remains missing after research and reconciliation,
create `GAPS.md` at the company-context root and set `gaps_report: GAPS.md` in the
root manifest. Treat it as authoring-only context, not as a runtime business fact.
For each gap record the affected artifact or field, sources checked, business impact,
owner or exact question, and recommended next action. Omit the file and manifest key
when no consequential gaps remain.
